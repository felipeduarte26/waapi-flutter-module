...
class SeniorMaskTextInputHelper implements TextInputFormatter {
  final SeniorMaskAutoCompletionType type;
  ...
  ///  - [SeniorMaskAutoCompletionType.lazy] (default): autocomplete unfiltered characters once the following filtered character is input.
  ...
  SeniorMaskTextInputHelper({
    ...
    String? initialText,
    this.type = SeniorMaskAutoCompletionType.lazy,
  }) {
  ...
  String maskText(String text) {
    return SeniorMaskTextInputHelper(mask: _mask, filter: _maskFilter, initialText: text).getMaskedText();
  }

  /// Unmask some text
  String unmaskText(String text) {
    return SeniorMaskTextInputHelper(mask: _mask, filter: _maskFilter, initialText: text).getUnmaskedText();
  }
  ...
  TextEditingValue formatEditUpdate(TextEditingValue oldValue, TextEditingValue newValue) {
    ...
    while (maskPos < mask.length) {
      ...
      } else if (!isMaskChar && !curTextInRange && type == SeniorMaskAutoCompletionType.eager) {
        curTextInRange = true;
      }
      ...
      if (isMaskChar && curTextInRange && curTextChar != null) {
        ...
        if (type == SeniorMaskAutoCompletionType.lazy || lengthRemoved > 0 || currentResultSelectionLength > 0) {
          nonMaskedCount++;
        }
      }
...
enum SeniorMaskAutoCompletionType {
  lazy,
  eager,
}
