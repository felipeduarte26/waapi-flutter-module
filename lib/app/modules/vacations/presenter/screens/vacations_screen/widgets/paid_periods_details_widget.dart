...
class PaidPeriodsDetailsWidget extends StatefulWidget {
  const PaidPeriodsDetailsWidget({
    Key? key,
  }) : super(key: key);

  @override
  State<PaidPeriodsDetailsWidget> createState() {
    return _PaidPeriodsDetailsWidgetState();
  }
}

class _PaidPeriodsDetailsWidgetState extends State<PaidPeriodsDetailsWidget> {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<VacationsScreenBloc, VacationsScreenState>(
      ...
      builder: (context, state) {
        ...
        final isLoading = (vacationsState is LoadingVacationsState && vacationsState.vacations == null);
        ...
        if (vacationsState is ErrorVacationsState) {
          return ErrorStateWidget(
            ...
            title: context.translate.paidPeriodsError,
            subTitle: context.translate.paidPeriodsErrorDescription,
        ...
        final paidVacations = vacationsState.vacations!.where((vacation) {
          return vacation.vacationPeriodSituation == VacationPeriodSituationEnum.paid;
        }).toList();

        if (paidVacations.isEmpty) {
          return EmptyStateWidget(
            title: context.translate.paidPeriodsEmpty,
            subTitle: context.translate.paidPeriodsEmptyDescription,
        ...
        return Scrollbar(
          child: ListView.separated(
            ...
            itemCount: paidVacations.length,
            separatorBuilder: (_, __) => const SizedBox(
              height: SeniorSpacing.normal,
            ),
            itemBuilder: (_, index) {
              final vacation = paidVacations[index];
              ...
              return Column(
                children: [
                  if (vacationsState is LoadedVacationsState)
                    if (index == 0)
                      _titleGroup(
                        title: context.translate.periodsPaid,
                      ),
                  WaapiCardWidget(
                    key: Key('vacations-paid_periods_details-list-card-$index'),
                    onTap: () => Modular.to.pushNamed(
                      VacationsRoutes.paidPeriodDetailsScreenInitialRoute,
                      arguments: {
                        'vacationPeriodId': vacation.vacationPeriodId,
                        'employeeId': (vacationsState as LoadedVacationsState).employeeId,
                      },
                    ),
                    child: Column(
                      ...
                      children: [
                        ...
                        if (vacationDays + vacationBonusDays != 0.0)
                          Padding(
                            padding: const EdgeInsets.only(
                            ...
                            child: SeniorText.body(
                              vacationBonusDays > 0
                                  ? context.translate.vacationsWithAllowance(
                                      StringHelper.doubleToStringFormatter(
                                        value: vacationDays + vacationBonusDays,
                                      ),
                                    )
                                  : context.translate.vacationsWithoutAllowance(
                                      StringHelper.doubleToStringFormatter(
                                        value: vacationDays + vacationBonusDays,
                                      ),
                                    ),
